...
bool PsychicHttpServer::TakeMutexForClients() {
  return (xSemaphoreTakeRecursive(_clientsMutex, portMAX_DELAY) == pdTRUE);
}

void PsychicHttpServer::GiveMutexForClients() {
  xSemaphoreGiveRecursive(_clientsMutex);
}
...
esp_err_t PsychicHttpServer::openCallback(httpd_handle_t hd, int sockfd)
{
  ...
  if (!server->TakeMutexForClients()) return ESP_FAIL;
...
void PsychicHttpServer::closeCallback(httpd_handle_t hd, int sockfd)
{
  ...
  if (!server->TakeMutexForClients()) return;
